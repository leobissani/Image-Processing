function zoomIn(arquivo)

    imagem = imread(arquivo);
    [H,W, colormap] = size(imagem);
    ...
    for i = 1:H %Loop for reading row and
        for t = 1:f %Row replication
            for j = 1:W %Loop for reading column and
    ...
    figure
    %subplot(1,2,1), 
    imshow(imagem), title('original(256,256)')
    %subplot(1,2,2), 
